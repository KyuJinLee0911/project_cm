...
def estimate_body_center(points):
    ...
    cx = head[0]*weights["head"] + torso[0]*weights["torso"] + arms[0]*weights["arms"] + legs[0]*weights["legs"]
    cy = head[1]*weights["head"] + torso[1]*weights["torso"] + arms[1]*weights["arms"] + legs[1]*weights["legs"]
...
def detect_people(
    ...
    for i, person_xy in enumerate(keypoints.xy):
        ...
        # 지지 다각형(손2+발2)
        polygon = np.array(
            [left_wrist, right_wrist, right_ankle, left_ankle],
            np.int32
        ).reshape((-1, 1, 2))

        # 무게중심 및 안정 여부
        body_center = estimate_body_center(pts)
        stable = cv2.pointPolygonTest(
        ...
        if draw:
            ...
            # 몸 중심
            cv2.circle(frame, (int(body_center[0]), int(body_center[1])), 6, (0, 0, 255), -1)
