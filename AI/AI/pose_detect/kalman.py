...
class KalmanKeypoints:
    ...
    def interpolate(self, idx, conf_th=0.4):
        ...
        prev_conf = self._history_conf[-2] if len(self._history_conf) > 1 else np.ones(self.num_kp)
...
# 한 사람 단순 래퍼
class KalmanTracker:
    def __init__(self, num_kp=17, dt=1/30.0, q=1e-4, r=1e-1, conf_th=0.4, occluded_damp=0.2):
    ...
    def update(self, pts, conf=None, dt=None):
        if dt is not None:
            self.set_dt(dt)
        out = self.kp_filter.update(pts, conf=conf, conf_th=self.conf_th, occluded_damp=self.occluded_damp)
        self.initialized = True
        self.last_pts = out
        return out
